class RA_SR3_Base : RifleBoltFree_Base
{
	...
	override SprayPaintItemEntry GetSprayPaintTypes()
	{
		...
		m_SprayPaintEntry.Add(SprayPaintTypes.BLACK, "RA_SR3_Black");
...
class RA_SR3: RA_SR3_Base
{
	...
	{	
		...
		if (Rotor_43.Cast(item))
		{
			SetObjectTexture(2,"RearmedServer\\Firearms\\ASVAL\\data\\Grip_co.paa");
		}
	}
	#endif
};

class RA_SR3_Black: RA_SR3_Base
{
	#ifndef SERVER
	override void EEItemDetached(EntityAI item, string slot_name)
	{	
		...
		if (Rotor_43.Cast(item))
		{
			SetObjectTexture(2,"RearmedServer\\Firearms\\ASVAL\\data\\Grip_Black_co.paa");
		}
	}
	#endif
};
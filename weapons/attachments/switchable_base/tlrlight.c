modded class TLRLight
{
	...
	protected ref array<PistollightLight> m_Lights = {};
	...
	override void OnWorkStart()
	{
		...
		for (int i = 0; i < LIGHT_COUNT; i++) {
			PistollightLight light = PistollightLight.Cast(ScriptedLightBase.CreateLight(PistollightLight, "0 0 0", 0.08)); // Position is zero because light is attached on parent immediately.
	...
	override void OnWork( float consumed_energy )
	{
		...
		foreach (PistollightLight light: m_Lights) {
			if (!light) {
				continue;
			}
